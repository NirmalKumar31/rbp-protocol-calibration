...
|---|---|
| Manuscript PDF | `manuscript/paper.pdf` (55 pages) |
| Abstract (paste into the form) | abstract of `manuscript/paper.tex`, 250 words, no markup |
...
one most likely to require revision is the 54-page main text with 17 table environments. The
abstract is now 250 words. `results/tables/PROVENANCE.csv` identifies which tables are secondary
and would move to a journal-specific supplement first.
...
then the declarations and 30 references.
...
2. **A journal submission after the preprint** may require a different main/supplement split.
   The current abstract is 250 words; any remaining length and structure changes depend on the
   target journal.
3. **Per-commit AI co-author trailers stop after `202953d`; disclosure moves to the
   ...
   quietly stopped.
4. **The AI-use disclosure has been rewritten for author review.** It names Anthropic Claude and
   OpenAI Codex and describes coding and editorial assistance. The
   unverifiable statement that no reference was generated by a language model has been removed.
   Before submission, the author must confirm that the paragraph in `paper.tex` accurately
   describes the assistance received and approve it as the disclosure of record.
