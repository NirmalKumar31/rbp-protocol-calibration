...
\item \textbf{Confirm that the baseline remains estimable as its order increases.} In this panel,
  out-of-fold baseline performance improved through order two but not at order four. Beyond that
...
by 0.020 AUROC, accounting for 46\% of its excess over the GC arm, and increased its measured
...
position or longer-range context. I interpret these analyses as a characterization of what the
...
current study does not test that generalization directly.
...
Neural-model initialization was not seeded across the 2830 committed fold runs. A repeated CNN
...
experiment combines hardware and initialization effects and does not establish complete neural
