...
giving 95 datasets over 79 proteins. Systematic sampling by rank rather than selection of the largest $N$ was used because AUROC
correlates with dataset size at $r=0.53$ to $0.67$ in these data, so a size-truncated panel
would confound panel membership with the measured quantity; the retained panel spans the 0th to
99th percentile of the candidate size distribution. Of the 95, 94
also cleared the floor in the GC arm and constitute the paired panel used for every
three-protocol result, giving 282 protocol-dataset cells; the model ladder uses all 95. Panel membership was written once to a manifest that every downstream stage reads. Supplementary Table S1 lists every dataset with its ENCFF file and ENCSR
...
Positive windows are 101~nt centred on the midpoint of each peak interval. Peak width and all significance columns are unused: no
$p$-value, $q$-value or fold-change filter was applied, so the positive set comprises every
called peak. Minus-strand windows are reverse-complemented
...
3$'$UTR, CDS, non-coding exon, intron) whose interval set contains the window midpoint. Region intervals were built from all transcripts with no
...
nominal tolerance, that is 0.15. The operative acceptance bound is therefore 0.15 rather than the nominal 0.05. Only the target protein's own
...
\emph{counts}, searched with a $k$-d tree over the forty nearest neighbours. Counts rather than frequencies are used
because integer distances are exactly representable, and each candidate carries an additional
...
cross-validation fold. These negatives are transcribed, accessible to crosslinking and strand-correct by
construction, and are not composition-matched in any respect.

\subsection{Achieved match quality}

Achieved matching was quantified over all 456{,}734 pairs per arm (Table~\ref{tab:match}).
...
The GC matcher holds its nominal tolerance for 94.8\% of pairs and its
...
out-of-fold score vector. Pooling rather than averaging per-fold AUROCs makes the
estimate invariant to fold size; the least balanced dataset places 0.42 of its data in a single
fold.

Leakage between folds was audited by exact 32-mer sharing rather than by clustered sequence
identity. A 101~nt window contains 70 distinct 32-mers, and two unrelated sequences share one
with negligible probability, so a shared 32-mer indicates common origin.
...
necessary here because the two score vectors are fitted on overlapping training data and
evaluated on identical rows, so they are strongly correlated and treating them as independent
understates the variance of their difference. Confidence intervals on a single dataset's
...
Three measures guard against platform-dependent results: dinucleotide distances are computed on
