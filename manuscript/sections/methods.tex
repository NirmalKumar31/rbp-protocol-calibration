...
training three times, and no additional data is spent on model selection. The 4-mer and both
nested logistic fits need no validation fold and train on all four non-test folds.
...
with negligible probability, and a shared 32-mer indicates common origin. Over all 94 datasets,
...
One channel remains open and is worth naming, because it is not removed by scoring out of fold.
...
Section~\ref{sec:estimands} is label-free and moves panel means by $4\times10^{-5}$. The bias is one-directional, since it can only help the score column, and it
...
estimator returns for such a model is reported in Section~\ref{sec:floor}: the floor reported there is an upper bound that
includes this channel along with the conditioning effect described in that section, and we do
