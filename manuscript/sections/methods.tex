...
which is necessary here because the discriminative signal is not at the window centre. Taking
the mutual information in bits between the base at each of the 101 positions and the label, over
every window of a dataset and with no model fitted, the position carrying the most information
sits a median of 23~nt from the midpoint, with an interquartile range of 11 to 29~nt and a
maximum of 48; it lies within 5~nt of the midpoint in only 13 of 94 datasets. The midpoint
...
harness runs 860 numeric assertions against a frozen expectations file, and the harness asserts
that no
