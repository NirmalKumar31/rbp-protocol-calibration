...
giving 95 datasets over 79 proteins. Systematic sampling by rank rather than selection of the
largest $N$ was used because AUROC correlates with dataset size in these data, negligibly for
...
pool is the study panel (40 to 48 donor proteins per dataset, median 45) rather than a full
ENCODE survey, and exclusion is by distance from the target's 101~nt windows rather than from
its full peak intervals. Sampling is performed within fold, so every pair stays inside one
...
knob, and the difference matters for how their results should be read. Both composition-matched
protocols draw candidates of the same transcript region class as the positive, so the region
label carries no information about the label: a classifier given only the five region dummies
scores exactly 0.5000 in both arms, in all 94 datasets. The bias-aware protocol matches fold
only. Its positive and negative region distributions differ at a median total variation distance
of 0.419, and the region label alone separates its two classes at a median AUROC of 0.748
(interquartile range 0.692 to 0.779, above 0.70 in 67 of 94 datasets). Only the bias-aware
...
the region asymmetry contributes, and the strand and transcription placebos below bound the
other two.
...
column is standardised over the whole dataset rather than within fold; the transform is
...
clustered windows as independent. We measured both components rather than assuming them
...
than 72 of 94 below. This is one of two quantities in the paper that the verification harness cannot check
...
harness runs 674 numeric assertions against a frozen expectations file, and the number of
