...
giving 95 datasets over 79 proteins. We sampled systematically by rank instead of taking the
largest $N$, because AUROC correlates with dataset size in these data, negligibly for
...
full peak intervals. Sampling is performed within fold, so every pair stays inside one
...
transcription placebos in the Results bound the other two.
...
harness runs 696 numeric assertions against a frozen expectations file, and the harness asserts
that no
