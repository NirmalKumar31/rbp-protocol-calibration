...
remove it rather than only measuring it. Deleting every held-out window that shares more than
half its 32-mers with a training window removes 0.22\% of windows and leaves the panel means at
$+0.0263$, $+0.0659$ and $+0.0122$, a span of 5.42. Deleting every held-out window that shares
\emph{any} 32-mer with a training window is the strongest form available: it removes all exact
cross-fold sharing, which the code asserts by re-indexing every 32-mer at stride one afterwards
and failing if one still crosses a fold. That costs 3.3\% of windows and gives $+0.0240$,
$+0.0618$ and $+0.0118$, a span of 5.24. The effect is smaller with the homology gone, by about
7\% of the contribution and 3\% of the span, and it is plainly still there.
...
harness runs 1012 numeric assertions against a frozen expectations file, and the harness asserts
that no
