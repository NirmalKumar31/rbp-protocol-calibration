...
nearest-neighbour search that minimises $L_1$ over all sixteen pins GC incidentally and tightly
for a typical pair, whereas the GC matcher performs a tolerance test and accepts the first
candidate inside it, leaving the achieved gap spread over the whole acceptance interval rather
than driven to zero. The tails invert for the complementary reason: the GC matcher imposes a
hard cap and so cannot exceed 0.15, while the dinucleotide matcher imposes no maximum distance
and assigns greedily, so a positive reached late in its group, once the nearby candidates are
used, takes the nearest remaining one however far away that is. The two arms therefore differ
