...
Section~\ref{sec:models} is label-free and moves panel means by at most $4\times10^{-5}$. The bias is one-directional, since it can only help the score column, and it
should grow with how much the base model overfits. We do not remove it; we measure it. A model
whose information the baseline already contains has a true increment of zero, and what the
estimator returns for such a model is reported in Section~\ref{sec:floor}: the floor reported there is what the estimator returns
for a 2-mer, and it therefore includes whatever this channel contributes for a model of that
kind, without separating it from the conditioning effect described in that section. It does not
bound the channel for a fine-tuned transformer: by the argument just given the bias grows with
how much the base model overfits, and a sixteen-feature model is the least overfitting case we
could have chosen. The calibration is a floor for the estimator, not a ceiling on the leak.
...
harness runs 937 numeric assertions against a frozen expectations file, and the harness asserts
that no
