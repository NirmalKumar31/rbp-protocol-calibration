...
control it two ways rather than only measuring it. Deleting every held-out window that shares
more than half its 32-mers with a training window removes 0.22\% of windows and moves the panel
means to $+0.0263$, $+0.0659$ and $+0.0122$, a span of 5.42 against the published 5.42. Pulling
every homology component that straddles two folds wholly into one of them, which tightens the
published partition rather than replacing it and moves a mean of 1.1\% of windows, gives
$+0.0265$, $+0.0662$ and $+0.0123$, a span of 5.40. Neither control is a sequence-identity
clustering with a tunable threshold, which remains the stronger instrument; both remove the
sharing that is actually present, and the protocol effect does not depend on it.
...
harness runs 1008 numeric assertions against a frozen expectations file, and the harness asserts
that no
