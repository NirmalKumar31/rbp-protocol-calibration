...
Extending both the sweep and the exponent family to the other two model classes, now that all
three have a bias-aware row, sharpens this and costs the headline its universality. The floor
over the eight transforms is 2.00 for the 4-mer but 2.91 for the convolutional network and 1.68
for SpliceBERT, so no model class reaches protocol independence among fixed coordinates and all
three sit above the equal-means null; but ``the floor is 2.00'' is the 4-mer's floor and we
report it as such. The fitted exponent also does better for the 4-mer than for anything else:
its argmin reaches 1.004, while the CNN's bottoms out at 1.213 and SpliceBERT's at 1.124, so a
fitted rescaling does not reach protocol independence for either neural model even in sample.
And the equalising exponent differs by a factor of 1.22 \emph{between model classes on the same
benchmark} (1.544, 1.843, 1.507), so it is not a property of the benchmark either; it is fitted
per measurement.
...
\paragraph{This subsection does not generalise across model classes, and that limits it.}
Everything above is measured with the 4-mer. Repeating it for the other two models on the same
rows:

\begin{table}[htbp]
\centering
\caption{The baseline-versus-label decomposition, per model class, $n=94$. The conclusion
reverses for the convolutional network, and the two-family mechanism does not survive for
SpliceBERT.}
...
CNN & \textbf{6.25\%} & \textbf{2.34\%} & $-0.120$ & $-0.410$ & $+0.075$ \\
SpliceBERT & 1.07\% & 24.06\% & $-0.583$ & $-0.767$ & $\mathbf{-0.568}$ \\
\bottomrule
\end{tabular}
\end{table}

Two things fail. For the convolutional network the ordering \emph{reverses}: the protocol label
explains 6.25\% against the baseline's 2.34\%, so ``it is the baseline, not the protocol label''
is false for that model. And the two-family mechanism is specific to the 4-mer: for SpliceBERT
the gradient is strong in all three arms including the bias-aware one ($-0.568$), where the
4-mer shows nothing ($-0.122$), so there is no two-family structure in that measurement.

We therefore state this subsection as what it is, a property of the 4-mer measurement rather
than of the quantity. The variance accounting, the natural experiment and the family mechanism
are reported for the 4-mer and should not be read as general. What does generalise across model
classes is the span itself (Table~\ref{tab:threemodels}), the absence of a protocol-independent
fixed coordinate, and the recommendation below.
...
This does generalise across model classes, which the baseline decomposition did not: rank
agreement improves in all three protocol pairs for all three models, with mean changes of
$+0.040$ (4-mer), $+0.094$ (CNN) and $+0.061$ (SpliceBERT).
