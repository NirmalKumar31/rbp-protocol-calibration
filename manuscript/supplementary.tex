...
\usepackage{caption}
% THE SECTION COLUMN BELOW IS RESOLVED FROM paper.aux, NOT TYPED. It was typed once, and six of
% its eleven rows pointed at the wrong section from the day the table was written: a reader
% sent to the region figure landed in 3.1, the compression figure in 3.2 and the baseline-order
% figure in 3.3. Nothing caught it, because a hand-typed number in a separate document is not a
% cross-reference and no build can check it. xr-hyper reads the paper's own labels, so an
% unresolvable one now fails build.sh's undefined-reference gate. It must load before hyperref.
\usepackage{xr-hyper}
\usepackage[colorlinks=true,allcolors=blue]{hyperref}
% build.sh always compiles paper.tex first, so paper.aux exists by the time this is read.
...
\midrule
Figure S1  & \texttt{f0\_panel\_overview}          & the panel and how it was drawn   & \ref*{sec:panel} \\
Figure S2  & \texttt{f1\_cost\_of\_matching}       & the two-protocol contrast        & \ref*{sec:threeprotocols} \\
Figure S3  & \texttt{f2\_four\_models}             & four model classes, one protocol & \ref*{sec:methods-models} \\
Figure S4  & \texttt{f3\_strand\_placebo}          & the strand control               & \ref*{sec:threeprotocols} \\
Figure S5  & \texttt{f4\_replication}              & cross-cell-line replication      & \ref*{sec:sensitivity} \\
Figure S6  & \texttt{f5\_size\_modification}       & effect modification by size      & \ref*{sec:sensitivity} \\
Figure S7  & \texttt{f6\_k\_sweep}                 & sensitivity to $k$               & \ref*{sec:methods-models} \\
Figure S8  & \texttt{f7\_region}                   & region heterogeneity             & \ref*{sec:region} \\
Figure S9  & \texttt{f8\_scale\_check}             & the compression correction       & \ref*{sec:models} \\
Figure S10 & \texttt{f13\_baseline\_order\_models}  & order-3 baseline by model class  & \ref*{sec:orderthree} \\
...
This is a measured property of each protocol: the baseline each protocol leaves for a sequence
model to improve on.
...
in the main text.
\textbf{Source}: \texttt{cost\_of\_matching.csv}, the earlier two-arm analysis, which is why panel
(b) prints $+0.0662$ where Table~\ref*{tab:three} of the paper reports $+0.0663$ from the
three-arm analysis. The GC arm agrees between the two to $1\times10^{-6}$; the
dinucleotide arm's panel means agree to $7\times10^{-5}$ while its per-dataset values differ by
a median of $0.0015$. Both tables are committed and separately gated, and no claim in the paper
...
The contrast is paired within dataset, and the horizontal axis uses the GC arm's pair count.
Arm sizes differ slightly because the two matchers retain different positives; dataset size
primarily affects precision rather than bias. It is reported here as effect
...
choice costs. The extended baseline adds 63 non-redundant trinucleotide-frequency columns, for
82 features in total.
