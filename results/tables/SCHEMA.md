...
**Every column of every table is listed in `COLUMNS.csv`, generated by
`scripts/column_dictionary.py`.** Read that for the specifics; this file explains the two shapes
that cover most of them.

135 tables, 1393 columns, 90 distinct schemas at the time of writing, and those three numbers are
the reason COLUMNS.csv exists. This document previously opened "124 CSVs, two shapes", of which
"44" were summary tables. An audit counted the tree and found 135, 88 and 57. Every figure was
hand-typed and every one had drifted, in the document whose job is to say what the columns are.
Counts here are now either generated or absent.
